...
import { LogOut, GraduationCap, LayoutDashboard, PlusCircle, Settings, ChevronRight } from 'lucide-react';
...
const DashboardLayout = () => {
  ...
  const navLinks = user.role === 'admin'
    ? [
      ...
      { name: 'New Task', path: '/admin/create', icon: <PlusCircle size={20} /> }
    ]
    : [
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col md:flex-row font-outfit overflow-hidden relative">
      ...
      {/* ── BACKGROUND GRAPHIC (Consistent with Login) ── */}
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
        {/* Large Decorative Letter 'D' for Dashboard (Bardot Influence) */}
        <div className="absolute top-[5%] right-[-5%] text-[45rem] font-serif font-black text-slate-50 leading-none select-none opacity-40">
          D
        </div>
      ...
      <aside className="hidden md:flex w-72 bg-white flex-shrink-0 flex-col pt-10 z-20 h-screen border-r border-slate-100 relative shadow-[20px_0_60px_-15px_rgba(0,0,0,0.02)]">
        ...
        <div className="flex items-center gap-3 px-8 mb-12">
          <h1 className="text-2xl font-serif font-black text-slate-900 tracking-tighter">eduTrack<span className="text-indigo-500">.</span></h1>
        </div>

        {/* Navigation */}
        <nav className="flex-1 px-4 space-y-2">
          {navLinks.map((link) => {
            const isActive = location.pathname === link.path || (link.path !== '/admin' && link.path !== '/student' && location.pathname.startsWith(link.path));

            return (
              <Link
                key={link.name}
                to={link.path}
                className={`flex items-center gap-3 px-5 py-4 rounded-[20px] transition-all duration-300 font-bold tracking-tight text-sm relative z-10 group ${isActive ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-900'
                  }`}
              >
                {isActive && (
                  <motion.div
                ...
                <div className={`relative z-10 transition-colors duration-300 ${isActive ? 'text-indigo-600' : 'text-slate-300 group-hover:text-slate-500'} transform group-hover:scale-110`}>
                  {link.icon}
                </div>
        ...
        <div className="p-4 mt-auto mb-4">
          <div className="bg-slate-50/80 backdrop-blur-sm border border-slate-100 rounded-[24px] p-4 flex items-center justify-between shadow-sm hover:shadow-md transition-all duration-300">
      ...
      <div className="md:hidden flex items-center justify-between px-6 py-5 bg-white/80 backdrop-blur-md sticky top-0 z-30 border-b border-slate-100">
        <div className="flex items-center gap-3">
          <h1 className="text-xl font-serif font-black text-slate-900 tracking-tighter">eduTrack<span className="text-indigo-500">.</span></h1>
        </div>
      ...
      <main className="flex-1 overflow-x-hidden w-full relative h-[100dvh] md:h-screen z-10">
        <div className="h-full overflow-y-auto px-6 py-8 md:px-12 md:py-12 lg:px-16 lg:py-16 max-w-7xl mx-auto w-full relative z-10 pb-32 md:pb-16 custom-scrollbar">
          <Outlet />
        </div>
      </main>

      {/* ----------------- MOBILE BOTTOM NAV (Clean Dark Design) ----------------- */}
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.name}
                to={link.path}
                className={`flex-1 flex flex-col items-center justify-center py-2.5 rounded-2xl transition-all duration-300 relative z-10 ${isActive ? 'text-white' : 'text-slate-500 hover:text-slate-300'}`}
              >
                {isActive && (
                  <motion.div
